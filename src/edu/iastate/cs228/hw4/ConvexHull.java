...
public class ConvexHull {
	...
	public ConvexHull(int n) throws IllegalArgumentException {
		...
		for (int i = 0; i < n; i++) {
			points[i] = new Point(random.nextInt(101) - 50,
	...
	public ConvexHull(String inputFileName) throws FileNotFoundException,
		...
		while (scan.hasNextLine()) {
			...
			while (scanLine.hasNextInt()) {
				int temp1 = scanLine.nextInt();
				int temp2 = scanLine.nextInt();
	...
	private void quickSortRec(int first, int last) {
		if (first < last) {
			...
			quickSortRec(first, q);
			quickSortRec(q + 1, last);
		}
	...
	private int partition(int first, int last) {
		...
		while (true) {
			i++;
			while (i < last && comp.compare(points[i], x) < 0)
				i++;
			j--;
			while (j > first && comp.compare(points[i], x) > 0)
				j--;
